"""
rag_chain.py - RAG Chain for DipeshBot (LLM + Retriever + Prompt)
==============================================================
This module handles the RAG chain for answering questions about Dipesh Padole's portfolio.
...
PROMPT_TEMPLATE = """
You are a knowledgeable AI assistant representing Dipesh Padole's portfolio and professional profile.
You provide accurate, engaging information about Dipesh's skills, projects, experience, and achievements.

Use ONLY the information provided in the context below to answer the question.
If the context does not contain enough information about the specific question, politely say:
"I don't have that specific information, but you can reach out to Dipesh directly at dipesh.padole@example.com or visit the GitHub profiles linked in my portfolio."

Do NOT make up facts or details not in the knowledge base.
Keep your answer clear, concise, engaging, and helpful.
If appropriate, include relevant links or contact information.
...
def load_llm(model: Optional[str] = None) -> BaseLanguageModel:
    ...
    no_prompt = (os.getenv("DIPESHBOT_NO_PROMPT") or "").strip().lower() in ("1", "true", "yes", "y")
    ...
    if not api_key:
        raise EnvironmentError(
            ...
            "Set it in DipeshBot/.env, or export it in your shell. "
            "For non-interactive runs, set DIPESHBOT_NO_PROMPT=1 to disable prompting."
        )
...
def query_rag(question: str, chain: SimpleRetrievalQA) -> dict:
    if not question or not question.strip():
        ...
        return {
            "answer": "Please enter a valid question about me or my portfolio.",
            "sources": [],
        }
